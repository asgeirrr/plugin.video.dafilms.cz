...
def play_film(film_id, title):
    ...
    try:
        ...
        # Try setResolvedUrl first (standard Kodi addon method)
        try:
            success = xbmcplugin.setResolvedUrl(_handle, True, play_item)
            xbmc.log(f"DAFilms: setResolvedUrl returned: {success}", xbmc.LOGINFO)

            # Give Kodi time to process
            xbmc.sleep(1000)

            # Check if playback actually started using the player instance
            if player.isPlaying():
                xbmc.log("DAFilms: Playback started successfully with setResolvedUrl", xbmc.LOGINFO)
                return
        except Exception as e:
            xbmc.log(f"DAFilms: setResolvedUrl failed: {str(e)}", xbmc.LOGWARNING)
